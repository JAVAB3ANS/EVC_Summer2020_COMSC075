...
class FunnyAccount { //The type Account is already defined ???
    ...
    public FunnyAccount(long number, double balance) {
        this.number = number;
...
class FunnySavingsAccount extends FunnyAccount {
    protected double apr; 

    public FunnySavingsAccount() {
        super();
        apr = 0.0;
    }

    public FunnySavingsAccount(long number, double balance, double apr) {  
    ...
    public String toString() {
        return String.format(super.toString() + "Interest Rate: %.2f percent", apr * 100);
...
class FunnyCreditCardAccount extends FunnyAccount {
    private double apr;
    private double creditLimit;

    public FunnyCreditCardAccount() {
        super();
        apr = 0.0;
        creditLimit = 0.0;
    }

    public FunnyCreditCardAccount(long number, double balance, double apr, double creditLimit) {  
        super(number, balance);
        this.apr = apr;
    ...
    // *** The assignment says to not change the credit limit if you are
    ...
    public String toString() {
        return String.format(super.toString() + "Credit Limit: $%.2f", creditLimit);
    }
...
public class TestAccountsEdited {
    public static void main(String[] args) {
        FunnyAccount[] testAccounts = {
            new FunnyAccount(1066, 7500.00), //two-argument constructor named Account
            new FunnySavingsAccount(30507, 4500.00, 1.5), //three-argument constructor named SavingsAccount
            new FunnyCreditCardAccount(51782737, 7000.00, 8, 1000.00), //four-argument constructor CreditCardAccount
            new FunnyCreditCardAccount(629553328, 1500.00, 7.5, 5000), //four-argument constructor CreditCardAccount
            new FunnyCreditCardAccount(4977201043L, -5000.00, 7, 10000) //four-argument constructor CreditCardAccount
        ...
        for (int i = 0; i < testAccounts.length; i++) {
            ...
            // *** Now test to see what kind of account we are
            // *** processing, and print Annual Interest or Monthly Payment
            // *** if account is of the proper type
            if (testAccounts[i] instanceof FunnySavingsAccount) {
                System.out.printf("Annual Interest: $%.2f\n",
                    ((FunnySavingsAccount) testAccounts[i]).calculateInterest());
            } else if (testAccounts[i] instanceof FunnyCreditCardAccount) {
                System.out.printf("Monthly Payment: $%.2f\n",
                    ((FunnyCreditCardAccount) testAccounts[i]).calculatePayment());
            }
